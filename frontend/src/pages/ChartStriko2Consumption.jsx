...
const ChartStriko2Consumption = () => {
  const [gas_kemarin_striko2, setGas_kemarin_striko2] = useState([]);
  const [gasConsumptionData, setGasConsumptionData] = useState([]);

  //Mengambil data gas sensor kemarin=========================================================
  useEffect(() => {
    getGas_kemarin_striko2();
  }, []);

  const getGas_kemarin_striko2 = async () => {
    try {
      const response = await axios.get(
        "http://localhost:5000/akhirharikemarinstriko2"
      );
...
export default ChartStriko2Consumption;
